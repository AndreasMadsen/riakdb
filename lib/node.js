...
Node.prototype.connect = function () {
  ...
  this._socket.once('close', function (hadError) {
    ...
    if (!hadError) self._jobError(new Error('connection closed'));
...
Node.prototype._jobError = function (error) {
  if (this.inuse) {
    if (this._job.method === 'message') {
      this._job.callback(error, null);
    } else if (this._job.method === 'stream') {
      this._job.stream.emit('error', error);
    }
  }
};
