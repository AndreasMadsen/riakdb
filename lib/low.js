  ...
  if (info.response === 'message') {
    if (empty) {
      RiakClientLow.prototype[methodName] = new Function(
        'callback', 'this._message("' + info.type + '", {}, callback);'
      );
    } else {
      RiakClientLow.prototype[methodName] = new Function(
        'data', 'callback', 'this._message("' + info.type + '", data, callback);'
      );
    }
  }
    ...
    if (empty) {
      RiakClientLow.prototype[methodName] = new Function(
        'return this._stream("' + info.type + '", {});'
      );
    } else {
      RiakClientLow.prototype[methodName] = new Function(
        'data', 'return this._stream("' + info.type + '", data);'
      );
    }
  }
});
